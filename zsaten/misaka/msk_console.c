...
void MSK_Refresh(int Line)
{
	...
	// draw scrollbar
	int TempNow = Console.CurrentConsoleLine - (30 - 2);
	if(TempNow < 0) TempNow = 0;
	int TempMax = Console.TotalConsoleLine - (30 - 2);
	if(TempMax < 0) TempMax = 0;

	float Pos = MSK_ScaleRange((float)TempNow, 0.0f, (float)TempMax, 1.0f, (float)(30 - 2));
	if(isnan(Pos)) Pos = (float)(30 - 2);

	mvwvline(Console.WindowMain, 1, COLS - 1, ACS_VLINE, 30 - 2);
